...
public class QuestionDAO extends BaseHibernateDAO {
	...
	public Question findById(entity.QuestionId id) {
		log.debug("getting Question instance with id: " + id);
		try {
			Question instance = (Question) getSession().get("entity.Question",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Question instance) {
		...
		try {
			List results = getSession().createCriteria("entity.Question")
